...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        out1o = F.relu(self.bn1(self.conv1(x)), inplace=True)
        out1 = F.max_pool2d(out1o, kernel_size=3, stride=2, padding=1)
        out2 = self.layer1(out1)
        out3 = self.layer2(out2)
        out4 = self.layer3(out3)
        out5 = self.layer4(out4)
        return out1o, out2, out3, out4, out5
...
class MFF(nn.Module):
    def __init__(self,inc=64):
        ...
        self.conv0 = nn.Conv2d(inc, inc, kernel_size=3, stride=1, padding=1)
        self.MFF0 = nn.Sequential(nn.AvgPool2d(kernel_size=2, stride=2),
                                  nn.Conv2d(inc, inc, kernel_size=3, stride=1, padding=1))
        self.MFF1 = nn.Sequential(nn.AvgPool2d(kernel_size=4, stride=4),
                                  nn.Conv2d(inc, inc, kernel_size=3, stride=1, padding=1))
        self.MFF2 = nn.Sequential(nn.AvgPool2d(kernel_size=8, stride=8),
                                  nn.Conv2d(inc, inc, kernel_size=3, stride=1, padding=1))
        self.outconv = nn.Conv2d(inc, inc, kernel_size=3, stride=1, padding=1)
    def forward(self, x):
        x_size = x.size()
        resl = x + self.conv0(x)
        y = self.MFF0(x)
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.MFF0 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, input1, input2=[0, 0, 0, 0]):
        out0_10 = F.relu(self.bn0(self.MFF0(input1[0] + input2[0])), inplace=True)
        out0 = F.interpolate(out0_10, size=input1[1].size()[2:], mode='bilinear')
        out1_20 = F.relu(self.bn1(self.MFF1(input1[1] + input2[1] + out0)), inplace=True)
        out1 = F.interpolate(out1_20, size=input1[2].size()[2:], mode='bilinear')
        out2_40 = F.relu(self.bn2(self.MFF2(input1[2] + input2[2] + out1)), inplace=True)
        out2 = F.interpolate(out2_40, size=input1[3].size()[2:], mode='bilinear')
        out3 = F.relu(self.bn3(self.MFF3(input1[3] + input2[3] + out2)), inplace=True)
...
class DecoderCR(nn.Module):  # 补偿和去余分支
    def __init__(self):
        ...
        self.MFF0 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1)
        ...
        self.outconv = nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1)
        self.outbn = nn.BatchNorm2d(64)
    def forward(self, input1):
        out0 = F.relu(self.bn0(self.MFF0(input1[0])), inplace=True)
        out0 = F.interpolate(out0, size=input1[1].size()[2:], mode='bilinear')
        out1 = F.relu(self.bn1(self.MFF1(input1[1]  + out0)), inplace=True)
        out1 = F.interpolate(out1, size=input1[2].size()[2:], mode='bilinear')
        out2 = F.relu(self.bn2(self.MFF2(input1[2] + out1)), inplace=True)
        out2 = F.interpolate(out2, size=input1[3].size()[2:], mode='bilinear')
        out3 = F.relu(self.outbn(self.outconv(F.relu(self.bn3(self.MFF3(input1[3] + out2)), inplace=True))))
...
class RCRNet(nn.Module):
    def __init__(self, cfg):
        ...
        #                             nn.ReLU(inplace=True), nn.Conv2d(64, 64, kernel_size=3, padding=1), nn.BatchNorm2d(64),
        #                             nn.ReLU(inplace=True), nn.Conv2d(64, 1, kernel_size=3, padding=1))
        ...
        self.outconv = nn.Conv2d(15, 1, 1)
        self.avg160 = nn.AvgPool2d(kernel_size=2, stride=2)
        self.initialize()

    def forward(self, x,depth, shape=None):
        out1o, out2, out3, out4, out5 = self.bkbone(x)
        out1o_d, out2_d, out3_d, out4_d, out5_d = self.bkbone_depth(depth)
        #out2, out3, out4, out5 = out2+out2_d, out3+out3_d, out4+out4_d, out5+out5_d
        ...
        out2d, out3d, out4d, out5d = self.conv2d(out2_d), self.conv3d(out3_d), self.conv4d(out4_d), self.conv5d(out5_d)
        del out2, out3, out4, out5
        out1o_80 = self.avg160(out1o)+ self.avg160(out1o_d)
        out2s, out3s, out4s, out5s = out2s+out2d, out3s+out3d, out4s+out4d, out5s+out5d
        ...
        out1 = torch.cat([outs1_80+out1o_80,outcom1,outred1], dim=1)
        ...
        out2 = torch.cat([outs2_80+out1o_80, outcom2,outred2], dim=1)
        ...
        out3 = torch.cat([outs3_80+out1o_80, outcom3, outred3], dim=1)
        ...
        out4 = torch.cat([outs4_80+out1o_80, outcom4, outred4], dim=1)
        del out2c4, out3c4, out4c4, out5c4,out2r4, out3r4, out4r4, out5r4
        outs5 = self.encoder(out4)
        outs5_80, outs5_40, outs5_20, outs5_10 = self.decoders([out5s, out4s, out3s, out2s], outs5)
        out2c5, out3c5, out4c5, out5c5 = torch.cat([out2c, outs5_80], dim=1), \
                                         torch.cat([out3c, outs5_40], dim=1), \
                                         torch.cat([out4c, outs5_20], dim=1), \
                                         torch.cat([out5c, outs5_10], dim=1)
        out2r5, out3r5, out4r5, out5r5 = torch.cat([out2r, outs5_80], dim=1), \
                                         torch.cat([out3r, outs5_40], dim=1), \
                                         torch.cat([out4r, outs5_20], dim=1), \
                                         torch.cat([out5r, outs5_10], dim=1)
        outcom5 = self.decoderc([out5c5, out4c5, out3c5, out2c5])
        outred5 = self.decoderr([out5r5, out4r5, out3r5, out2r5])
        out5 = torch.cat([outs5_80+out1o_80, outcom5, outred5], dim=1)
        #del out2c4, out3c4, out4c4, out5c4,out2r4, out3r4, out4r4, out5r4
        #
        # outs6 = self.encoder(out5)
        # outs6_80, outs6_40, outs6_20, outs6_10 = self.decoders([out5s, out4s, out3s, out2s], outs6)
        # out2c6, out3c6, out4c6, out5c6 = torch.cat([out2c, outs6_80], dim=1), \
        #                                  torch.cat([out3c, outs6_40], dim=1), \
        #                                  torch.cat([out4c, outs6_20], dim=1), \
        #                                  torch.cat([out5c, outs6_10], dim=1)
        # out2r6, out3r6, out4r6, out5r6 = torch.cat([out2r, outs6_80], dim=1), \
        #                                  torch.cat([out3r, outs6_40], dim=1), \
        #                                  torch.cat([out4r, outs6_20], dim=1), \
        #                                  torch.cat([out5r, outs6_10], dim=1)
        # outcom6 = self.decoderc([out5c6, out4c6, out3c6, out2c6])
        # outred6 = self.decoderr([out5r6, out4r6, out3r6, out2r6])
        # out6 = torch.cat([outs6_80+out1o_80, outcom6, outred6], dim=1)
        if shape is None:
            shape = x.size()[2:]
        ...
        out5 = F.interpolate(self.linear(out5), size=shape, mode='bilinear')
        outcom5 = F.interpolate(self.linearc(outcom5), size=shape, mode='bilinear')
        outred5 = F.interpolate(self.linearr(outred5), size=shape, mode='bilinear')

        out = self.outconv(torch.cat((out1 ,outcom1, outred1, out2,outcom2, outred2, out3, outcom3, outred3, out4,outcom4, outred4, out5,outcom5, outred5), 1))
        #out1o_80 = F.interpolate(out1o_80, size=shape, mode='bilinear')
        return out1 ,outcom1, outred1, out2,outcom2, outred2, out3, outcom3, outred3, out4,outcom4, outred4, out5,outcom5, outred5,out
